...
void SQLiteDatabase::Open(const char *filename, int flags, const char *zVfs)
{
    ...
    strStream << "sqlite3_open_v2\n";
    std::string copyOfStr = strStream.str();
    printf("%s", copyOfStr.c_str());
    #endif // DEBUG
    // close old db, if one exist
    if(mDatabaseHandle)
        Close();
    int retCodeM = sqlite3_config( SQLITE_CONFIG_MEMSTATUS, 0 );
    if (retCodeM != SQLITE_OK)
    {
        printf("setting sqlite SQLITE_CONFIG_MEMSTATUS 0 !!! return code: %d\n", retCodeM);
    }
